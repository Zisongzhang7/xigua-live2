...
const LiveStreamCard: React.FC<LiveStreamCardProps> = ({ stream, onClick, onDelete }) => {
  const [showMenu, setShowMenu] = useState(false);
  const isLive = stream.status === LiveStatus.LIVE;

  const getRecentSessionInfo = () => {
    if (!stream.sessions || stream.sessions.length === 0) {
      return '未配置直播场次';
    }

    const now = new Date();
    // Sort sessions by startTime ascending
    const sortedSessions = [...stream.sessions].sort((a, b) =>
      new Date(a.startTime).getTime() - new Date(b.startTime).getTime()
    );

    // Find the first future or currently running session
    const upcomingSession = sortedSessions.find(s => new Date(s.startTime) > now);

    if (upcomingSession) {
      // Format time: MM-DD HH:mm
      const date = new Date(upcomingSession.startTime);
      ...
      return `最近开播: ${timeStr} ${upcomingSession.hostName}`;
    }

    // All sessions are in the past
    // Count sessions that have ended
    const endedCount = sortedSessions.filter(s => new Date(s.startTime) <= now).length;
    return `${endedCount}场直播已结束`;
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-2 relative">
        ...
        <div className={`text-[10px] font-bold px-2 py-0.5 rounded w-fit text-white ${stream.type === LiveType.COURSE ? 'bg-blue-600' : 'bg-emerald-600'
          }`}>
          ...
          <div className="relative">
            ...
            {showMenu && (
              ...
              >
                ...
                <button
                  className="w-full px-4 py-2 text-xs text-left text-red-600 hover:bg-red-50 flex items-center gap-2 border-t border-gray-50"
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowMenu(false);
        ...
        {stream.type === LiveType.ORDINARY && (
          <div className="flex items-center gap-1 text-[10px] text-gray-400 border-t border-gray-100 pt-2 mt-1">
            <Calendar size={12} />
            <span>{getRecentSessionInfo()}</span>
          </div>
        )}
      </div>
    </div>
