...
const STORAGE_KEYS = {
  STREAMS: 'app_data_streams',
  RESOURCES: 'app_data_resources',
  TEMPLATES: 'app_data_templates'
};
...
class DatabaseService {
  ...
  private init() {
    // 只有当本地没有数据时才写入初始数据，防止覆盖用户修改
    if (!localStorage.getItem(STORAGE_KEYS.STREAMS)) {
      localStorage.setItem(STORAGE_KEYS.STREAMS, JSON.stringify(INITIAL_STREAMS));
    }
  ...
  getStreams(): LiveStream[] {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.STREAMS) || '[]');
  }

  saveStream(stream: LiveStream) {
  ...
  // --- Resources ---
  getResources(): InteractiveResource[] {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.RESOURCES) || '[]');
  }

  saveResource(resource: InteractiveResource) {
  ...
  // --- Templates ---
  getTemplates(): InteractionTemplate[] {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.TEMPLATES) || '[]');
  }

  saveTemplate(template: InteractionTemplate) {
